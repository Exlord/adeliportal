...
class WorkshopTime extends BaseAbstractActionController
{
    private $workshop;
    private $class;
    ...
    public function indexAction()
    {
        ...
        $grid = new DataGrid('ec_workshop_timetable');
        $grid->route = 'admin/educational-center/workshop/class/timetable';
        $grid->routeParams['workshop'] = $this->workshop;
        $grid->routeParams['class'] = $this->class;
        ...
        $date = new \DataView\Lib\Date('date', 'Date');
        $date->sortable = true;
...
//        $timeBeforeUncancel = time() + (48 * 60 * 60); //TODO get from config
//        $timeBeforeCancel = time() + (48 * 60 * 60); //TODO get from config

        $workshop = $this->workshop;
        $classItem = $this->class;
        $now = time();

        $status = new Custom('status', 'Status',
            function (Custom $col) use ($workshop, $classItem, $now) {
                $s = $col->dataRow->status;
                $title = '';
                $icon = '';
                if ($s == '0') {
                    if ($col->dataRow->start > time()) {
                        $title = t('EC_TIMETABLE_WAITING');
                        $icon = 'glyphicon glyphicon-question-sign text-info';
                    } elseif ($col->dataRow->start <= time() && $col->dataRow->end >= time()) {
                        $title = t('EC_WORKSHOP_TIME_IN_PROGRESS');
                        $icon = 'glyphicon glyphicon-time text-primary';
                    } else {
                        $title = t('EC_WORKSHOP_TIME_DONE');
                        $icon = 'glyphicon glyphicon-ok-sign text-success';
                    }
                } elseif ($s == '1') {
                    $title = t('EC_WORKSHOP_TIME_CANCELED');
                ...
                if (isAllowed(Module::TIMETABLE_CHANGE_STATUS_ALL) || $col->dataRow->educatorId == current_user()->id) {
                    $icon = '';
                    $url = '#';
                    $class = 'btn btn-default btn-xs';
                    $title = '';
                    $route = false;
                    $routeParams = array('id' => $col->dataRow->id, 'workshop' => $workshop, 'class' => $classItem);
                    if ($s == '0' && $col->dataRow->start > $now) {
                        $title = t('EC_WORKSHOP_TIME_CANCEL');
                        $icon = 'glyphicon glyphicon-ban-circle text-danger';
                        $route = 'admin/educational-center/workshop/class/timetable/change-status';
                        $routeParams['status'] = 1;
                        $class .= ' ajax_page_load';
                    } elseif ($s == '1' && $col->dataRow->start > $now) {
                        $title = t('EC_WORKSHOP_TIME_UNCANCEL');
                        ...
                        if ($col->dataRow->start <= $now && $col->dataRow->end >= $now) {
                            $title = t('Session is in progress and cannot be changed.');
                            $icon = 'glyphicon glyphicon-time text-primary';
                            $url = 'javascript:System.AjaxMessage("' . $title . '")';
                        } elseif ($col->dataRow->end < $now) {
                            $title = t('Session has finished and cannot be changed.');
                            ...
                            $title = t('The time allowed for changing the session has passed and this session cannot be changed anymore.');
        ...
        $grid->addColumns(array($id, $date, $start, $end, $sessionTime, $timeDistance, $status, $del));
        $grid->addDeleteSelectedButton();
        $grid->addNewButton('New Time');

        $grid->getSelect()
            ->where(array($grid->getTableGateway()->table . '.classId' => $this->class));
        ...
        $this->viewModel->setTemplate('educational-center/workshop-time/index');
    ...
    public function newAction()
    {
        ...
        if (!$type) {
            $this->viewModel->setTemplate('educational-center/workshop-time/new-type');
            $this->viewModel->setVariables(array('workshop' => $this->workshop, 'class' => $this->class));
            return $this->viewModel;
        }

        $form = new \EducationalCenter\Form\WorkshopTime($type);

        $form->setAction(url('admin/educational-center/workshop/class/timetable/new', array('workshop' => $this->workshop, 'class' => $this->class, 'type' => $type)));
        $form->setAttribute('data-cancel', url('admin/educational-center/workshop/class/timetable', array('workshop' => $this->workshop, 'class' => $this->class,)));
...
//                            $end_date = mktime(0, 0, 0, $mo, $d, $y);
                            $end_date = Date::fromDatePicker($end_date);
                            //endregion
...
//                            $current_date = $start_date = mktime(0, 0, 0, $mo, $d, $y);
                            $current_date = $start_date = Date::fromDatePicker($start_date);
                            //endregion
                    ...
                    if (count($dates)) {
                        ...
                        foreach ($dates as $model) {
                            $model['status'] = 0;
                            $model['classId'] = $this->class;
                            if ($this->getTable()->save($model))
                                $savedCount++;
                        }
                        if ($savedCount) {
                            $this->flashMessenger()->addSuccessMessage(sprintf(t('%s workshop class time table entry created successfully'), $savedCount));
        ...
        $this->viewModel->setTemplate('educational-center/workshop-time/new');
    ...
    {
        $class = $this->class = $this->params()->fromRoute('class', false);
        $this->workshop = $this->params()->fromRoute('workshop', false);
...
//            $timeBeforeCancel = time() + (48 * 60 * 60); //TODO get from config
            ...
            elseif ($col->status == '0' && $col->start > $now && $status == '1') {
                $this->flashMessenger()->addSuccessMessage(t('Selected Session Canceled'));
                $process = true;
            ...
            elseif ($col->status == '1' && $col->start > $now && $status == '0') {
                $this->flashMessenger()->addSuccessMessage(t('Selected Session Uncanceled'));
                $process = true;
            }
    ...
    {
        if ($this->request->isPost()) {
            $idList = $this->params()->fromPost('id', 0);
            if ($idList) {
                if (!is_array($idList))
                    $idList = array($idList);
                foreach ($idList as $id)
                    $this->notify($id);
                $this->getTable()->remove($idList);
    ...
    private function notify($class)
    {
        if ($notifyApi = getNotifyApi()) {
            ...
            if ($class) {
                ...
                $notifyApi->notify('EducationalCenter', 'workshop_class_timetable_changed', array(
     ...
     */
    private function getWorkshopTable()
    {
