...
class Config extends BaseForm implements InputFilterProviderInterface
{
    ...
    public function __construct($baseType,$isRequest)
    {
        ...
        parent::__construct('third_config');
        $this->setAttribute('class', 'normal-form ajax_submit');
        $this->setAttribute('action', url('admin/ad/config/third-config'));
    }

    protected function addElements()
    {
        $this->add(new SortAdsCollection($this->baseType,$this->isRequest));

        $this->add(new \System\Form\Buttons('third_config'));
    }
