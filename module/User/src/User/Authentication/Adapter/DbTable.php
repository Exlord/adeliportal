...
class DbTable extends AbstractAdapter implements \Zend\ServiceManager\ServiceLocatorAwareInterface
{
     ...
     * @return \User\Authentication\Adapter\DbTable
     */
     ...
     *
     * @throws \Zend\Authentication\Adapter\Exception\RuntimeException if answering the authentication query is impossible
     ...
     * @throws \Zend\Authentication\Adapter\Exception\RuntimeException when an invalid select object is encountered
     * @return \User\Model\User
     */
    protected function _authenticateQuerySelect(DbSelect $dbSelect)
    {
        $getBy = 'getBy' . ucfirst($this->identityColumn);

        try {
            $resultIdentities = $this->getServiceLocator()->get('user_table')->{$getBy}($this->getIdentity())->toArray();
        } catch (\Exception $e) {
...
//        unset($resultIdentity['zend_auth_credential_match']);
        ...
        if ($resultIdentity['accountStatus'] != '1') {
            $this->authenticateResultInfo['code'] = 'DISABLED';
            $this->authenticateResultInfo['messages'][] = 'This account is ' . UserTable::$accountStatus[$resultIdentity['accountStatus']];
            return $this->_authenticateCreateAuthResult();
        }

        $bcrypt = new Bcrypt();
//        $securePass = $bcrypt->create($this->getCredential());
